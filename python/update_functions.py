...
def fromJsonApiToDf(datetime):
    ...
    df = df.astype({"idsensore": int, "valore": float, "idoperatore": int, "stato": str})
    df["dataora"] = pd.to_datetime(df["dataora"], format="%Y/%m/%d %H:%M:%S")
    with open('../dati_sensori_1968_2020/test_update/%s_data.json' %str(date.today()), 'w') as outfile: 
...
def UpdateCheckFinal():
    files = glob.glob("../dati_sensori_1968_2020/test_update/*.json")
    ...
    with open("../dati_sensori_1968_2020/test_update/comparisons.txt", 'a') as f:
