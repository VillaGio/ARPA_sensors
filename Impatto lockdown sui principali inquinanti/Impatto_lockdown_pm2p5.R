...
dati<-read.csv(file ="C:\\Users\\stefa\\Tesi\\serie_pm2p5.csv")
...
lambdal<-BoxCox.lambda(serie, method = "loglik") # -0.15
lambdag<-BoxCox.lambda(serie, method = "guerrero") # -0.23 direi di usare il logaritmo
...
legend("top",leg.txt,bty="n",lwd=2,col=c("blue","red","green"))
...
t.test(dati_dest, pre, "less")# no non cambia
