...
dati<-read.csv(file ="C:\\Users\\stefa\\Tesi\\serie_pm10.csv")
summary(dati)
...
dati_impatto<-dati[6940:8400,]

# Rendere i dati stazionari -----------------------------------------------

lambdal<-BoxCox.lambda(serie, method = "loglik") # 0
lambdag<-BoxCox.lambda(serie, method = "guerrero") # 0.09 direi di usare il logaritmo
...
# provare con regressione lineare fino al 8 marzo 2020, fare previsioni fino a fine 2021 e confrontare le
# medie poi fare un test
...
tr<-lm(Valore~Data, data=dati_postlock)
summary(tr)
plot(dati_postlock$Data,dati_postlock$Valore, type='l', col="blue", lwd=2, 
     main="Post lockdown period, estimated trend against real trend and values",
...
legend("topleft",leg.txt,bty="n",lwd=2,col=c("blue","red","green"))
...
t.test(dati_postlock$Valore, pre, "less") # p-value = 0.544
# vedila come vuoi però a me non sembra proprio che il lockdown sia stato influente o forse no?

# Dati Destagionalizzati, concettualmente migliore ------------------------

dati_dest<-diff(dati_prelock$Valore,365)
appo<-dati_prelock$Data[0:7373]
...
abline(tr, col="green", lwd=2)
leg.txt<-c("Real Values","Estimated Trend","Real Trend")
legend("bottom",leg.txt,bty="n",lwd=2,col=c("blue","red","green"))
...
t.test(dati_dest, pre, "greater")# col lockdown il pm10 aumenta
