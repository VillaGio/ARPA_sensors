...
dati<-read.csv(file ="C:\\Users\\stefa\\Tesi\\serie_O3.csv")
...
lambdal<-BoxCox.lambda(serie, method = "loglik") # 0.07
lambdag<-BoxCox.lambda(serie, method = "guerrero") # 0.35 direi di usare il logaritmo
...
t.test(dati_postlock$Valore, pre, "greater") # p-value = 0.5
# Il lockdown è stato insignificativo  come pochi
...
legend("bottomleft",leg.txt,bty="n",lwd=2,col=c("blue","red","green"), cex=0.9)
...
t.test(dati_dest, pre, "less")# non cambia nulla
...
legend("topleft",leg.txt,bty="n",lwd=2,col=c("blue","green","salmon2","red"), cex=0.7)
