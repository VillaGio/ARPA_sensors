...
d2021NO <- d2021[d2021$NomeTipoSensore=="Ossidi di Azoto",]
d2021NO <- d2021NO[d2021NO$valore > 0,]

rm(d2021)
rm(dati2021)

d2021NO_g <- d2021NO_grouped <- d2021NO %>%
  group_by(idsensore, lat, lng) %>%
  summarise_at(vars(valore), list(name = mean))

## costruisco la mappa

coordinates(d2021NO_grouped)=c("lng","lat")
...
idw.p=idw(formula=log(name) ~ 1, locations=d2021NO_grouped , newdata=grid, 
          nmax = 10, idp = 2)
...
d2019NO <- d2019[d2019$NomeTipoSensore=="Ossidi di Azoto",]
d2019NO <- d2019NO[d2019NO$valore > 0,]

rm(d2019)
rm(dati2019)

d2019NO_g <- d2019NO_grouped <- d2019NO %>%
  group_by(idsensore, lat, lng) %>%
  summarise_at(vars(valore), list(name = mean))

## costruisco la mappa

coordinates(d2019NO_grouped)=c("lng","lat")

idw.p=idw(formula=log(name) ~ 1, locations=d2019NO_grouped , newdata=grid, 
          nmax = 10, idp = 2)
idw.o=as.data.frame(idw.p)
names(idw.o)[1:3]<-c("long","lat","logp.pred")
...
d2017NO <- d2017[d2017$NomeTipoSensore=="Ossidi di Azoto",]
d2017NO <- d2017NO[d2017NO$valore > 0,]

rm(d2017)
rm(dati2017)

d2017NO_g <- d2017NO_grouped <- d2017NO %>%
  group_by(idsensore, lat, lng) %>%
  summarise_at(vars(valore), list(name = mean))

## costruisco la mappa

coordinates(d2017NO_grouped)=c("lng","lat")

idw.p=idw(formula=log(name) ~ 1, locations=d2017NO_grouped , newdata=grid, 
          nmax = 10, idp = 2)
idw.o=as.data.frame(idw.p)
names(idw.o)[1:3]<-c("long","lat","logp.pred")
...
d2015NO <- d2015[d2015$NomeTipoSensore=="Ossidi di Azoto",]
d2015NO <- d2015NO[d2015NO$valore > 0,]

rm(d2015)
rm(dati2015)

d2015NO_g <- d2015NO_grouped <- d2015NO %>%
  group_by(idsensore, lat, lng) %>%
  summarise_at(vars(valore), list(name = mean))

## costruisco la mappa

coordinates(d2015NO_grouped)=c("lng","lat")

idw.p=idw(formula=log(name) ~ 1, locations=d2015NO_grouped , newdata=grid, 
          nmax = 10, idp = 2)
idw.o=as.data.frame(idw.p)
names(idw.o)[1:3]<-c("long","lat","logp.pred")
...
d2013NO <- d2013[d2013$NomeTipoSensore=="Ossidi di Azoto",]
d2013NO <- d2013NO[d2013NO$valore > 0,]

rm(d2013)
rm(dati2013)

d2013NO_g <- d2013NO_grouped <- d2013NO %>%
  group_by(idsensore, lat, lng) %>%
  summarise_at(vars(valore), list(name = mean))

## costruisco la mappa

coordinates(d2013NO_grouped)=c("lng","lat")

idw.p=idw(formula=log(name) ~ 1, locations=d2013NO_grouped , newdata=grid, 
          nmax = 10, idp = 2)
idw.o=as.data.frame(idw.p)
names(idw.o)[1:3]<-c("long","lat","logp.pred")
...
d2011NO <- d2011[d2011$NomeTipoSensore=="Ossidi di Azoto",]
d2011NO <- d2011NO[d2011NO$valore > 0,]

rm(d2011)
rm(dati2011)

d2011NO_g <- d2011NO_grouped <- d2011NO %>%
  group_by(idsensore, lat, lng) %>%
  summarise_at(vars(valore), list(name = mean))

## costruisco la mappa

coordinates(d2011NO_grouped)=c("lng","lat")

idw.p=idw(formula=log(name) ~ 1, locations=d2011NO_grouped , newdata=grid, 
          nmax = 10, idp = 2)
idw.o=as.data.frame(idw.p)
names(idw.o)[1:3]<-c("long","lat","logp.pred")
